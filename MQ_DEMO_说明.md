...
### 死信队列
观察控制台或 `logs/dead-letter-queue.log`：
```
========================================
【死信队列告警】收到死信消息
========================================
...
### Q: 如何调整重试次数？
A: RocketMQ不支持直接配置重试次数。业务逻辑中可以根据reconsumeTimes判断是否继续重试或直接丢弃。

### Q: 如何手动处理死信消息？
A: 可以通过RocketMQ控制台或API重新发送死信消息，也可以在死信消费者中实现自动重发逻辑。

### Q: 死信消息会丢失吗？
A: 不会，除非显式删除。RocketMQ会持久化死信消息。
